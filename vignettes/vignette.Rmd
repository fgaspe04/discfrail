...
In Section 5 of @gasperoni2018nonparam , this model is used with an outcome of the time until readmission to hospital for patients with heart failure that are grouped by their healthcare providers. 
...
* $p_{cens}$, the proportion of events which are censored.
...
Exploiting equations \eqref{invthm} and \eqref{survtime} we are able to simulate the _event_ times $T_{ij}$. Then, we generate the _censoring_ times according to a Normal distribution, independently from the $T_{ij}$.   The mean of the distribution is taken to be the $1 - p_{cens}$ quantile of the simulated event times  [ TODO explain variance ] so that $p_{cens}$ of the simulated times are censored. 
...
* `sim_weibdf`, a parametric baseline, specifically a Weibull distribution characterised by two parameters $\lambda$ and $\rho$, with cumulative hazard function [ TODO write explicitly as function of the parameters ] 
...
The function `npdf_cox` can be used to fit the Cox model with nonparametric discrete frailty.  This is based on maximum partial likelihood using an EM algorithm -- see Section 3 of @gasperoni2018nonparam.  We illustrate the use of this function to fit this model to the dataset `data_weib` that was simulated previously. 
...
We can investigate the posterior probabilities of being part of a specific latent population ($\alpha_{jk}$, see Section 3.1 of @gasperoni2018nonparam).  For some groups there is great uncertainty about the latent population to which they belong (i.e., group 1 is assigned to latent population 1 and 2 with probabilities of 0.59, 0.41 respectively).    The following code identifies those groups for which no latent group has a posterior probability of more than 0.85 -- note that this includes the three "misclassified" groups highlighted above. 
...
assign_not_sure1 = which( test_weib$models[[2]]$alpha[ ,1 ] < 0.85 &
                          test_weib$models[[2]]$alpha[ ,2 ] < 0.85 )
assign_not_sure1
```
